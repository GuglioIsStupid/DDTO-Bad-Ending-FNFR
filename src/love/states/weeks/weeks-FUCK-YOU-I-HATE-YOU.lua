...
return {
	...
	generateNotes = function(self, chart)
		...
		for i = 1, #chart do
			...
			for j = 1, #chart[i].sectionNotes do
				...
				if noteVer == "default" or not noteVer then
					if settings.downscroll then
				...
				elseif noteVer == "Sayo Sing" then
					if not mustHitSection then
						local id = noteType + 1
						local c = #sayoNotes[id] + 1
						local x = sayoArrows[id].x

						table.insert(sayoNotes[id], sprite())
						sayoNotes[id][c].x = x
						sayoNotes[id][c].y = -400 + noteTime * 0.6 * speed

						sayoNotes[id][c]:animate("on", false)
						...
						if chart[i].sectionNotes[j].noteLength > 0 then
							...
							for k = 71 / speed, chart[i].sectionNotes[j].noteLength, 71 / speed do
								...
								table.insert(sayoNotes[id], sprite())
								sayoNotes[id][c].x = x
								sayoNotes[id][c].y = -400 + (noteTime + k) * 0.6 * speed
								if k > chart[i].sectionNotes[j].noteLength - 71 / speed then
									sayoNotes[id][c].offsetY = 10

									sayoNotes[id][c]:animate("end", false)
								else
									sayoNotes[id][c]:animate("hold", false)
								end
							end
				...
				elseif noteVer == "Yuri Sing" then
					if mustHitSection then
						local id = noteType - 3
						local c = #yuriNotes[id] + 1
						local x = yuriArrows[id].x

						table.insert(yuriNotes[id], sprite())
						yuriNotes[id][c].x = x
						yuriNotes[id][c].y = -400 + noteTime * 0.6 * speed

						yuriNotes[id][c]:animate("on", false)
						...
						if chart[i].sectionNotes[j].noteLength > 0 then
							...
							for k = 71 / speed, chart[i].sectionNotes[j].noteLength, 71 / speed do
								...
								table.insert(yuriNotes[id], sprite())
								yuriNotes[id][c].x = x
								yuriNotes[id][c].y = -400 + (noteTime + k) * 0.6 * speed
								if k > chart[i].sectionNotes[j].noteLength - 71 / speed then
									yuriNotes[id][c].offsetY = 10

									yuriNotes[id][c]:animate("end", false)
								else
									yuriNotes[id][c]:animate("hold", false)
								end
							end
		...
		if settings.downscroll then
			...
			table.sort(sayoNotes[i], function(a, b) return a.y > b.y end)
			table.sort(yuriNotes[i], function(a, b) return a.y > b.y end)
		end
			...
			if musicThres ~= oldMusicThres and math.fmod(absMusicTime, 120000 / bpm) < 100 then
				...
				if song == 3 then
					if spriteTimers[4] == 0 then
						self:safeAnimate(sayori, "idle", false, 4)
					end
					if spriteTimers[5] == 0 then
						self:safeAnimate(yuri, "idle", false, 5)
					end
				end
			end
		...
		if not paused then
			if not doingDialogue then
				...
				for i = 1, 4 do
					...
					local sayoNote = sayoNotes[i]
					local sayoArrow = sayoArrows[i]
					local yuriNote = yuriNotes[i]
					local yuriArrow = yuriArrows[i]
					...
					if #sayoNote > 0 then
						if not countingDown then
							if (sayoNote[1].y - musicPos <= -400) then
								sayoArrow:animate("confirm", false)

								if sayoNote[1]:getAnimName() == "hold" or sayoNote[1]:getAnimName() == "end" then
									if (not sayori:isAnimated()) or sayori:getAnimName() == "idle" then self:safeAnimate(sayori, curAnim, true, 4) end
								else 
									self:safeAnimate(sayori, curAnim, false, 2)
								end
							end
							table.remove(sayoNote, 1)
						end
					end
					if #yuriNote > 0 then
						if not countingDown then
							if (yuriNote[1].y - musicPos <= -400) then
								yuriArrow:animate("confirm", false)

								if yuriNote[1]:getAnimName() == "hold" or yuriNote[1]:getAnimName() == "end" then
									if (not yuri:isAnimated()) or yuri:getAnimName() == "idle" then self:safeAnimate(yuri, curAnim, true, 5) end
								else 
									self:safeAnimate(yuri, curAnim, false, 2)
								end
							end
							table.remove(yuriNote, 1)
						end
					end
		...
		love.graphics.push()
			...
			for i = 1, 4 do
				...
				love.graphics.push()
					...
					for j = #sayoNotes[i], 1, -1 do
						if sayoNotes[i][j].y - musicPos <= 560 then
							local animName = sayoNotes[i][j]:getAnimName()

							sayoNotes[i][j]:draw()
						end
					end
					for j = #yuriNotes[i], 1, -1 do
						if yuriNotes[i][j].y - musicPos <= 560 then
							local animName = yuriNotes[i][j]:getAnimName()
	...
	drawHealthBar = function()
		love.graphics.push()
			...
			elseif accForRatingText >= 90 then -- KYS!
				ratingText = "Great!"
